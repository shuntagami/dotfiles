...
今日の画面記録、音声記録、休憩ログ、今日の宣言タスクを確認し、Slackに日次ワークレポートを投稿してください。
...
このPipeの目的は、1日の終わりに「何時間働いたか」「何に時間を使ったか」「朝に宣言したタスクがどこまで進んだか」「明日何を続けるか」を短く可視化することです。
...
- 今日の宣言タスクは `~/.screenpipe/work-state/daily-goals/YYYY-MM-DD.json` に記録されています。
...
- 今日の0:00から現在までを対象にする。
- まず `~/dotfiles/bin/screenpipe-daily-goals context` を実行して、今日の宣言タスクを読む。
...
- 朝に宣言したタスクが、達成・一部達成・着手済み・未着手・判定不能のどれか
- 明日に残ったことは何か
- 休憩を除いた実作業時間はどれくらいか
- 集中から大きくそれた時間があったか
...
宣言タスクがある場合は、各タスクを次の5段階で判定してください。

- `達成`: `today done` で完了済み、または画面・音声・作業ログから完了の根拠が強い。
- `一部達成`: 主要な前進はあるが、タスク全体の完了までは読み取れない。
- `着手済み`: 関連する作業対象に触れているが、成果や完了は弱い。
- `未着手`: 今日の記録に関連する作業痕跡がほぼない。
- `判定不能`: Screenpipeの記録だけでは判断できない。

無理に達成扱いしないでください。`done` されていないタスクで完了根拠が弱い場合は `一部達成` または `着手済み` に留めてください。
...
- 「宣言タスク」は、宣言がある場合だけ書く。例: `達成1 / 一部2 / 未着手1`。可能なら重要な1件だけ短く補足する。
...
```bash
"$HOME/dotfiles/bin/screenpipe-daily-goals" context
```

このJSONを、以後のアクティビティ分析で必ず参照する。`goals` が空なら、宣言タスクの行は投稿から省略してよい。
...
この結果からSlack投稿の内容を組み立てる。宣言タスクの達成度、成果、未完了作業が不明な場合のみステップ3へ進む。

# ステップ3: 追加確認検索

アクティビティ概要だけでは、宣言タスクの達成度・進んだこと・未完了・明日の作業を判断しきれない場合のみ実行する。
