...
`~/.screenpipe/work-state/work.jsonl` に作業ログがあり、現在作業中ではないと分かる場合は、画面内容に関係なく通知しない。

勤務外は、YouTubeやSNSを見ていても脱線とは扱わない。
...
function isWorkActive() {
  if (!fs.existsSync(workFile)) return false;
  ...
  for (const event of events) {
    if (event.type === "work_start") active = true;
    if (event.type === "work_end") active = false;
  }
  return active;
}
